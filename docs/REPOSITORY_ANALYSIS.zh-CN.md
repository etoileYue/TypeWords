...
TypeWords 是一个基于 `pnpm workspace` 的 TypeScript/Vue monorepo，用于英语单词、文章跟打、听写、自测和学习进度管理。仓库提供：
...
仓库中 `apps` 与 `packages` 下主要由 TypeScript、Vue SFC 和少量 JavaScript 脚本组成。
...
- `packages/core` 承担绝大多数业务能力，Nuxt 主站通过共享组件和 store 复用逻辑。
